            ...
            <table class="min-w-full text-left text-sm font-light text-surface dark:text-white tw:mr-10px" name="Tabla de Reservas">
                ...
                <tbody id="personRowsContainer">
                    <tr class="person-row">
                        ...
                        <td class="tw:text-center">
                            <input type="text" name="persons[0][gender]" class="tw:w-48">
                        </td>
                        <td class="tw:text-center">
                            <input type="number" name="persons[0][age]" class="tw:w-40">
                        </td>
                        <td class="tw:text-center">
                        ...
                        <td class="tw:text-center">
                            <input type="text" name="persons[0][seat_cost]" class="tw:w-40" readonly>
                        </td>
                    </tr>
                </tbody>
            </table>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        addAnotherBtn.addEventListener('click', function() {
            ...
            // Clona la última fila existente para mantener los selectores con valores por defecto
            // Si prefieres clonar siempre la primera fila, usa: personRowsContainer.children[0].cloneNode(true);
            const originalRow = personRowsContainer.lastElementChild;
            if (!originalRow) return;

            const newRow = originalRow.cloneNode(true); // Clonar en profundidad (con todos sus hijos)

            // Limpiar los valores de los inputs en la nueva fila y ajustar los atributos 'name'
            newRow.querySelectorAll('input, select').forEach(input => {
                ...
                // Asegurarse de que el campo 'seat_cost' siempre sea de solo lectura
                if (input.name.includes('[seat_cost]')) {
                    input.setAttribute('readonly', 'true');
                } else {
                    input.removeAttribute('readonly'); // Asegurarse de que los otros inputs no sean readonly si se clonó de una fila que sí lo era (aunque en tu HTML no es el caso)
                }
            });
